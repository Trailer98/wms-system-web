<template>
  <section class="page-shell">
    <div class="page-header">
      <div>
        <h2>客户管理</h2>
        <p>维护客户基础资料，支持按客户编码和名称查询。</p>
      </div>
      <el-button v-if="authStore.hasPermission('customer:create')" type="primary" @click="openCreateDialog">新增客户</el-button>
    </div>
    ...
    <CommonDataTable
      ...
      empty-text="暂无客户数据"
      @pagination-change="fetchCustomers"
    >
    ...
    <el-dialog v-model="createDialogVisible" title="新增客户" width="520px">
      <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-width="90px">
        <el-form-item label="客户编码" prop="code">
          <el-input v-model.trim="createForm.code" maxlength="64" show-word-limit placeholder="请输入客户编码" />
        </el-form-item>
        <el-form-item label="客户名称" prop="name">
          <el-input v-model.trim="createForm.name" maxlength="128" show-word-limit placeholder="请输入客户名称" />
        </el-form-item>
...
const queryFields = [
  { prop: 'code', label: '客户编码', type: 'input', placeholder: '请输入客户编码', trim: true },
  { prop: 'name', label: '客户名称', type: 'input', placeholder: '请输入客户名称', trim: true }
]

const tableColumns = [
  { prop: 'code', label: '客户编码', minWidth: 130 },
...
const fetchCustomers = async () => {
  loading.value = true
  try {
    const response = await axios.get('/customers', { params: buildParams() })
